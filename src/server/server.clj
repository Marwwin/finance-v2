...
(jdbc/execute! ds ["CREATE TABLE IF NOT EXISTS finance (id INTEGER PRIMARY KEY AUTOINCREMENT, state TEXT)"])
...
(defn get-current-state [_]
  (let  [state (jdbc/execute! ds ["SELECT state FROM finance ORDER BY id DESC LIMIT 1"])]
    {:status 200
     :headers headers
